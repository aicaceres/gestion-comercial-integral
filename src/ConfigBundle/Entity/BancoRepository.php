...
class BancoRepository extends EntityRepository {
    ...
    public function findMovimientoCheque($chequeId) {
        $query = $this->_em->createQueryBuilder();
        $query->select('m')
            ->from('ConfigBundle:BancoMovimiento', 'm')
            ->innerJoin('m.tipoMovimiento', 't')
            ->innerJoin('m.cheque', 'c')
            ->where("t.nombre = 'CHEQUE'")
            ->andWhere('c.id =' . $chequeId);
        return $query->getQuery()->getOneOrNullResult();
    }

    public function findBancosRetencion() {
        $query = $this->_em->createQueryBuilder();
        $query->select('b.nombre')
            ->from('ConfigBundle:Banco', 'b')
            ->where("b.nombre LIKE 'RETENCION%'")
            ->andWhere('b.activo = 1');
        return $query->getQuery()->getArrayResult();
    }
