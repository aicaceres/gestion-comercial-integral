                ...
                if ($entity->getDescuentaStock()) {
                    ...
                    elseif ($difDetalle) {
                        ...
                        foreach ($detalleNuevo as $det) {
                            ...
                            foreach ($detalleHistorico as $key => $hist) {
                                if ($det->getProducto()->getId() == $hist->getProducto()->getId()) {
                                    $found = true;
                                    // correccion del stock
                                    $diferencia = $hist->getCantidad() - $det->getCantidad();
                                    if ($diferencia != 0) {
                                        $stock = $em->getRepository('AppBundle:Stock')->findProductoDeposito($det->getProducto()->getId(), $entity->getDeposito()->getId());
                                        $stock->setCantidad($stock->getCantidad() + $diferencia);
                                        $em->persist($stock);
                                        $em->flush();
                                    }
                                    // correccion de cantidad en historico
                        ...
                        foreach ($detalleHistorico as $key => $hist) {
                            ...
                            if (!$found) {
                                $stock = $em->getRepository('AppBundle:Stock')->findProductoDeposito($hist->getProducto()->getId(), $entity->getDeposito()->getId());
                                $stock->setCantidad($stock->getCantidad() + $hist->getCantidad());
                                $em->persist($stock);
                                $em->flush();
                                $em->remove($hist);
                            }
